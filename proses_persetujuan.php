...
$aksi = $_GET['aksi'] ?? null;
if (!$pengajuan_id || !$aksi) {
    header("Location: administrasi_cuti.php?status=gagal");
    exit();
}
$conn->begin_transaction();
try {
    $status_baru = ($aksi == 'setujui') ? 'Disetujui' : 'Ditolak';
    $id_penanggung_jawab = $_SESSION['user_id'];
    $stmt_update_status = $conn->prepare("UPDATE pengajuan_cuti SET status_pengajuan = ?, diproses_oleh = ? WHERE id = ?");
    $stmt_update_status->bind_param("sii", $status_baru, $id_penanggung_jawab, $pengajuan_id);
    ...
    $conn->commit();
    header("Location: administrasi_cuti.php?status=sukses");
    exit();
} catch (Exception $e) {
    $conn->rollback();
    header("Location: administrasi_cuti.php?status=gagal");
